...
fn a_trained_field_concentrates_on_the_taught_direction() {
    ...
    assert!(
        aligned > 350,
        "only {aligned} of 500 samples within 60° of the taught direction"
    );
    assert!(
        aligned >= first,
        "more training must not blur the peak: {first} -> {aligned}"
    );
...
fn the_guided_pdf_integrates_to_one() {
    ...
    for _ in 0..5000 {
        ...
        data.push(SampleData {
            pos: Vec3A::splat(0.5),
            dir: d,
            radiance: if z > 0.0 { 3.0 } else { 1.0 },
        });
    }
    f.update(&data, 1);
    let p = Vec3A::splat(0.5);
...
fn training_is_spatially_local() {
    ...
    assert!(
        f.pdf(a, Vec3A::Z) > f.pdf(a, -Vec3A::Z),
        "corner a should favour +Z"
    );
    assert!(
        f.pdf(b, -Vec3A::Z) > f.pdf(b, Vec3A::Z),
        "corner b should favour -Z"
    );
}
...
fn field_is_cloneable_and_debuggable() {
    ...
    let c = f.clone();
    assert_eq!(
        c.trained_at(Vec3A::splat(0.5)),
        f.trained_at(Vec3A::splat(0.5))
    );
    assert_eq!(
        c.pdf(Vec3A::splat(0.5), Vec3A::X),
        f.pdf(Vec3A::splat(0.5), Vec3A::X)
    );
    let _ = format!("{f:?}");
    let s = SampleData {
        pos: Vec3A::ZERO,
...
fn degenerate_bounds_are_padded() {
    ...
    let mut f = GuidingField::new(
        AABB::new(Vec3A::ZERO, Vec3A::new(1.0, 0.0, 1.0)),
        GuidingConfig::default(),
    );
    let data: Vec<SampleData> = (0..500)
        .map(|i| SampleData {
            pos: Vec3A::new((i % 10) as f32 / 10.0, 0.0, 0.5),
            dir: Vec3A::Y,
            radiance: 1.0,
        })
        .collect();
